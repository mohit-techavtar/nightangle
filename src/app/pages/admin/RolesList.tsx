...
import {
  ...
  Users, Lock, Eye, Plus, ChevronDown, AlertTriangle, X, CheckCircle2
} from "lucide-react";
...
export function RolesList() {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      await deleteRole(roleToDelete.id, `Role "${roleToDelete.name}" deleted by admin`);
      await loadRoles();
  ...
  const filteredRoles = roles.filter(role => {
    ...
    const matchesFilter = selectedFilter === "all" ||
      ...
      (selectedFilter === "critical" && role.riskLevel === "critical") ||
      (selectedFilter === "high" && role.riskLevel === "high");
    ...
    return (
      <div className="h-screen flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-sm text-[#64748B]">Loading roles...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="border-b border-gray-200 bg-white sticky top-0 z-10">
        <div className="px-8 py-6">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="font-semibold text-[#0F1B2D] mb-1">All Roles</h1>
              <p className="text-sm text-[#64748B]">
                Manage and configure role-based access control across the platform
              </p>
            </div>
            <div className="flex items-center gap-3">
              <button
                onClick={() => navigate("/admin/roles-permissions/matrix")}
            ...
            <tbody className="divide-y divide-gray-200">
              {filteredRoles.map((role) => (
                <tr key={role.id} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${role.isSystem ? "text-purple-600 bg-purple-50" : "text-blue-600 bg-blue-50"}`}>
                  ...
                  <td className="px-6 py-4">
                    <div className="flex items-center gap-1.5 text-sm text-[#0F1B2D]">
                      ...
                      {Object.values(role.permissions).reduce((acc, module) => {
                        return acc + Object.values(module || {}).reduce((sum, actions) => sum + (actions?.length || 0), 0);
                      }, 0)}
                    </div>
                  </td>
                  <td className="px-6 py-4">
                    <div className="flex flex-wrap gap-1">
                      {Object.keys(role.permissions).slice(0, 2).map((module, index) => (
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded text-xs font-medium ${getRiskColor(role.riskLevel)}`}>
                      ...
                      {openMenuId === role.id && (
                        <>
                          ...
                          <div className="absolute right-0 top-full mt-1 w-48 bg-white rounded-lg shadow-lg border border-gray-200 py-1 z-20">
                            <button
                              onClick={() => {
                                navigate(`/admin/roles-permissions/edit?id=${role.id}`);
                                setOpenMenuId(null);
                              }}
          ...
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-sm text-[#64748B] mb-6">
                Are you sure you want to delete <strong>{roleToDelete.name}</strong>? This will remove the role from{" "}
                <strong>{roleToDelete.userCount} user{roleToDelete.userCount > 1 ? 's' : ''}</strong>. This action cannot be undone.
              </p>
              <div className="flex items-center gap-3">
                <button
